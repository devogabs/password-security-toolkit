...
def generate_password(length: int = 12, use_upper: bool = True, use_lower: bool = True, use_digits: bool = True, use_symbols: bool = True) -> str:
    ...
    for _ in range(max_attempts):
        ...
        password = ''.join(secrets.choice(character_pool) for _ in range(
            length))  # list comprehension to build the password
    ...
    raise ValueError(
        "Failed to generate a strong password after multiple attempts.")
